...
def trans_i_to_v_test() -> None:
    ...
    # Expected outputs
    outputs = [np.array([[-27],[16],[-146]])]

    for input_it, output_it in zip(inputs, outputs):
        calculated_output = trans_i_to_v(**input_it)
...
def trans_v_to_i_test() -> None:
    ...
    # Expected outputs
    outputs = [np.array([[57],[180],[242]])]

    for input_it, output_it in zip(inputs, outputs):
        calculated_output = trans_v_to_i(**input_it)
...
def trans_i_to_b_test() -> None:
    ...
    inputs = [{'pose': Pose(), 'p_i': np.array([[15],[98],[48]])}]
    # Expected outputs
    outputs = [np.array([[-97.73759005],[55.3890873],[-98.3890873]])]

    for input_it, output_it in zip(inputs, outputs):
        calculated_output = trans_i_to_b(**input_it)
...
def trans_b_to_i_test() -> None:
    ...
    # Expected outputs
    outputs = [np.array([[89.82233047],[-16.17766953],[179.60660172]])]

    for input_it, output_it in zip(inputs, outputs):
        calculated_output = trans_b_to_i(**input_it)
