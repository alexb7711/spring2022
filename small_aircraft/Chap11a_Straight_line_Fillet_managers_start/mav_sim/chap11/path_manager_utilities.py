...
def inHalfSpace(pos: NP_MAT, hs: HalfSpaceParams) -> bool:
    ...
    # Local variables
    p = pos        # Position
    r = hs.point   # Point on half plane
    n = hs.normal  # Normal point from half plane

    # Calculate vector from position to point
    w = p - r

    # Determine whether in half plane
    return True if np.dot(w[0,:],n[0,:]) >= 0 else False
