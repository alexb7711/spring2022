...
def derivatives(state: types.DynamicState, forces_moments: types.ForceMoment) -> types.DynamicState:
    ...
    # position kinematics
    pos_dot = Quaternion2Rotation(state[IND.QUAT]) @ state[IND.VEL]
    north_dot = pos_dot.item(0)
    east_dot = pos_dot.item(1)
    down_dot = pos_dot.item(2)

    # position dynamics
    u_dot = st.r*st.v - st.q*st.w + fm.fx/MAV.mass
    v_dot = st.p*st.w - st.r*st.u + fm.fy/MAV.mass
    w_dot = st.q*st.u - st.p*st.v + fm.fz/MAV.mass

    # rotational kinematics
    e0_dot = 0.5 * (-st.p*st.e1 - st.q*st.e2 - st.r*st.e3)
    e1_dot = 0.5 * (st.p*st.e0 + st.r*st.e2 - st.q*st.e3)
    e2_dot = 0.5 * (st.q*st.e0 - st.r*st.e1 + st.p*st.e3)
    e3_dot = 0.5 * (st.r*st.e0 + st.q*st.e1 -st.p*st.e2)

    # rotatonal dynamics
    p_dot = MAV.gamma1*st.p*st.q - MAV.gamma2*st.q*st.r + MAV.gamma3*fm.l + MAV.gamma4*fm.n
    q_dot = MAV.gamma5*st.p*st.r - MAV.gamma6*(st.p**2-st.r**2) + fm.m/MAV.Jy
    r_dot = MAV.gamma7*st.p*st.q - MAV.gamma1*st.q*st.r + MAV.gamma4*fm.l + MAV.gamma8*fm.n

    # collect the derivative of the states
    x_dot = np.empty( (IND.NUM_STATES,1) )
    x_dot[IND.NORTH] = north_dot
    x_dot[IND.EAST] = east_dot
    x_dot[IND.DOWN] = down_dot
    x_dot[IND.U] = u_dot
    x_dot[IND.V] = v_dot
    x_dot[IND.W] = w_dot
    x_dot[IND.E0] = e0_dot
