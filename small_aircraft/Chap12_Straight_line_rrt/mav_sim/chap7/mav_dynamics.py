...
def calculate_sensor_readings(state: types.DynamicState, forces: types.NP_MAT, \
    ...
    st = DynamicState(state) # extract the states

    # Extract needed orientation values
    phi, theta, _ = Quaternion2Euler(state[IND.QUAT])

    # simulate rate gyros(units are rad / sec)
    sensors.gyro_x, sensors.gyro_y, sensors.gyro_z = gyro(st.p, st.q, st.r, noise_scale)
    ...
    sensors.mag_x, sensors.mag_y, sensors.mag_z = magnetometer(quat, noise_scale)

    # simulate pressure sensors
    sensors.abs_pressure, sensors.diff_pressure = pressure(st.down, Va, noise_scale)
    ...
    if update_gps:
        ...
        # Calculate the gps
        sensors.gps_n, sensors.gps_e, sensors.gps_h, sensors.gps_Vg, sensors.gps_course = \
            gps(state[0:3], state[IND.VEL], quat, nu_update, noise_scale)
...
def accelerometer(phi: float, theta: float, forces: types.NP_MAT, noise_scale: float ) -> tuple[float, float, float] :
    ...
    """
    # Implementation of equation (7.3)
    fx = forces.item(0)
    fy = forces.item(1)
    fz = forces.item(2)
    accel_x = fx/MAV.mass \
                        + MAV.gravity*np.sin(theta) \
                        + noise_scale * np.random.normal(0., SENSOR.accel_sigma)
    accel_y = fy/MAV.mass \
...
def gyro(p: float, q: float, r: float, noise_scale: float = 1. ) -> tuple[float, float, float] :
    ...
    """
    # Implementation of equation (7.5) (note that eta has a bias term and a normal noise component)
    gyro_x = p + SENSOR.gyro_x_bias \
                + noise_scale * np.random.normal(0., SENSOR.gyro_sigma)
    gyro_y = q + SENSOR.gyro_y_bias \
                + noise_scale * np.random.normal(0., SENSOR.gyro_sigma)
    gyro_z = r + SENSOR.gyro_z_bias\
                + noise_scale * np.random.normal(0., SENSOR.gyro_sigma)
...
def pressure(down: float, Va: float, noise_scale: float = 1. ) -> tuple[float, float] :
    ...
    """
    # Implementation of equation (7.9)
    abs_pressure = -MAV.rho * MAV.gravity * down + SENSOR.abs_pres_bias \
                        + noise_scale * np.random.normal(0., SENSOR.abs_pres_sigma)

    # Implementation of equation (7.10)
    diff_pressure = 0.5 * MAV.rho * Va**2 + SENSOR.diff_pres_bias \
                        + noise_scale * np.random.normal(0., SENSOR.diff_pres_sigma)
...
def magnetometer(quat_b_to_i: types.Quaternion, noise_scale: float = 1. ) -> tuple[float, float, float] :
    ...
    # Calculate the rotation from inertial frame to magnetic north frame.
    R_inertial_to_mag = Euler2Rotation(0.0, SENSOR.mag_inc, SENSOR.mag_dec)

    # Calculate the inertial unit magnetic field vector
    mag_inertial = R_inertial_to_mag.T @ np.array([[1.0], [0.0], [0.0]])

    # Calculate the body-frame unit magnetic field vector
    R_b_to_i = Quaternion2Rotation(quat_b_to_i) # body to inertial
    mag_body = R_b_to_i.T @ mag_inertial

    # Add zero mean white noise using a variance of SENSOR.mag_sigma
    mag_x = mag_body.item(0) + noise_scale * np.random.normal(0., SENSOR.mag_sigma)
    mag_y = mag_body.item(1) + noise_scale * np.random.normal(0., SENSOR.mag_sigma)
    mag_z = mag_body.item(2) + noise_scale * np.random.normal(0., SENSOR.mag_sigma)
...
def gps_error_trans_update(nu: GpsTransient, noise_scale: float = 1. ) -> GpsTransient :
    ...
    # Calculate the update (equation (7.17) )
    nu_n = np.exp(-SENSOR.gps_k*SENSOR.ts_gps)*nu.n \
                            + noise_scale * SENSOR.ts_gps * np.random.normal(0., SENSOR.gps_n_sigma)
    nu_e = np.exp(-SENSOR.gps_k*SENSOR.ts_gps)*nu.e \
                        + noise_scale * SENSOR.ts_gps * np.random.normal(0., SENSOR.gps_e_sigma)
    nu_h = np.exp(-SENSOR.gps_k*SENSOR.ts_gps)*nu.h \
                        + noise_scale * SENSOR.ts_gps * np.random.normal(0., SENSOR.gps_h_sigma)
...
def gps(position: types.NP_MAT, V_g_b: types.NP_MAT, e_quat: types.Quaternion, nu: GpsTransient, \
    ...
    # Calculate position measurement ( (7.18) thru (7.20) )
    ...
    # Calculate velocity measurement (Equation (7.21) with added noise)
    gps_Vg = np.sqrt(v_n**2 + v_e**2) + \
                        + noise_scale * np.random.normal(0., SENSOR.gps_Vg_sigma)

    # Calculate the course measurement ( Equation (7.22) with added noise)
    gps_course = np.arctan2(v_e, v_n) \
                        + noise_scale * np.random.normal(0., SENSOR.gps_course_sigma)
